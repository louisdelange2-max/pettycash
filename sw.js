const CACHE_NAME = "petty-cash-pwa-v7-fcm-fix";
const APP_SHELL = [
  "./",
  "./index.html",
...
messaging.onBackgroundMessage(function(payload) {
  const title = (payload && payload.notification && payload.notification.title) || 'Petty Cash App Live';
  const options = {
    body: (payload && payload.notification && payload.notification.body) || 'New petty cash activity.',
    icon: './icons/icon-192.png',
    badge: './icons/icon-192.png',
    data: {
      url: (payload && payload.data && payload.data.url) || './',
      transactionId: (payload && payload.data && payload.data.transactionId) || ''
    }
  };
